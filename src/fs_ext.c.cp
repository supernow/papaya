...
#define BTL(block_id) (g_dp[currfd.device][currfd.partition].start_lba+BTS(block_id))
#define ROOT_INODE 2
//gdt
...
static int loadpart(){
	...
	oprintf("startseek:%u,endseek:%u,start_block:%u,end_block:%u,seek_in_startblock:%u,seek_in_endblock:%u\n",startseek,endseek,start_block,end_block,seek_in_startblock,seek_in_endblock);
	...
	for(i=0;i<12;i++){
		...
		if(curr_block>=start_block){
			if(curr_block==start_block){//必须先判断是不是start_block，因为start_block可能=end_block
				...
				int realsize=BLOCK_SIZE-seek_in_startblock;
				memcp(_load_addr,fsbuf+seek_in_startblock,realsize);	
				_load_addr+=realsize;
			}
